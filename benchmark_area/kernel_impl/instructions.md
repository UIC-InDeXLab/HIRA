...
with S=16 I could scann only 16% that should result in speedup. consider this: in benchmarking/kernel_impl, my goal is to make search faster. Here is the idea I    
  want to apply: during the build, after finding the parents for children (kcenter clustering on each subspace), reorder the children such that the child of parent i 
   is in children[bf*i:bf*(i+1)]. So that there is no scatter or second access to an index array to find out the children. for each parent, only extend parents if    
  that parent passes the filter (AND logic) on all the subspaces.
you may also need to have a v2.0 build that does the kcenter in a balanced way: all parents have at most bf children. consider fixed bf=4 if it helps in fast compiled kernels.
  implement this search in a fused kernel that parallelizes on parents and also on subspaces (so that the overhead of having multiple subspace and multiple parents   
  is reduced).
  use ~/venv/bin if necessary. Also go for cuda kernels if they can result in faster (comaprable to cuBLAS bmm). the parallalization is key. try ideas on that. 

write new search versions for this.