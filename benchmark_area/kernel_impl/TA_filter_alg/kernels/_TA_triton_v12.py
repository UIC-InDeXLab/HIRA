"""v12 Triton helpers for S=4, bf=4 TA attention experiments.

Adds two pieces missing from the v11 helper set:

* depth/selected extraction with separate order row stride and selected-mask K;
* duplicate-corrected merged subspace cluster-stream attention.
"""
...
if HAS_TRITON:
    ...
    @triton.jit
    def _ta_v12_depth_selected_kernel(
        Order_ptr,         # (H_q, 4, ROW_STRIDE) int64/int32
        SortedScores_ptr,  # (H_q, 4, ROW_STRIDE) fp16/fp32, descending
        Threshold_ptr,     # (H_q,) fp32
        Depth_ptr,         # (H_q,) int32
        Selected_ptr,      # (H_q, 4, K_CLUSTERS) int8
        K_CLUSTERS,
        ROW_STRIDE,
        ROWS,
        ...
        rows_valid = offs < ROWS
        k_valid = offs < K_CLUSTERS
        ...
        below = (row_sum < th) & rows_valid
        first = tl.min(tl.where(below, offs, ROWS), axis=0)
        depth = tl.where(first < ROWS, first + 1, ROWS)
        tl.store(Depth_ptr + hq, depth)

        for s_idx in tl.static_range(4):
            sel_base = (hq * 4 + s_idx) * K_CLUSTERS
            tl.store(
                Selected_ptr + sel_base + offs,
                tl.zeros([K_BLOCK], dtype=tl.int8),
                mask=k_valid,
            )
            in_top = rows_valid & (offs < depth)
            clusters = tl.load(
                Order_ptr + (hq * 4 + s_idx) * ROW_STRIDE + offs,
                mask=in_top,
                other=0,
            ).to(tl.int32)
            c_valid = in_top & (clusters >= 0) & (clusters < K_CLUSTERS)
            tl.store(
                Selected_ptr + sel_base + clusters,
                tl.full([K_BLOCK], 1, dtype=tl.int8),
                mask=c_valid,
            )

    @triton.jit
    def _ta_v12_corrected_cluster_attn_kernel(
        Q_ptr,               # (H_q, D) fp16
        Order_ptr,           # (H_q, 4, ROW_STRIDE) int64/int32
        Depth_ptr,           # (H_q,) int32
        ...
        for r_chunk in range(r_start, r_stop, BLOCK_ROWS):
            ...
            clusters = tl.load(
                Order_ptr
                + (hq * SUBS + sub_inner[None, :]) * ROW_STRIDE
                + rows_safe[:, None],
                mask=row_valid[:, None],
                other=0,
            ).to(tl.int32)
            cluster_valid = row_valid[:, None] & (clusters >= 0) & (clusters < K_CLUSTERS)
            ...
            for p in tl.static_range(4):
                ...
                p_valid = owned & (parent_p >= 0) & (parent_p < K_CLUSTERS)
                sel = tl.load(
                    Selected_ptr + (hq * 4 + p) * K_CLUSTERS + parent_p,
                    mask=p_valid,
                    other=0,
                )
                multiplicity += tl.where(sel != 0, 1, 0)

            mult_safe = tl.maximum(multiplicity, 1).to(tl.float32)
            ...
            if tl.max(owned.to(tl.int32), axis=0) != 0:
                ...
                pvals = tl.exp2(scaled - m_new) / mult_safe
...
def run_ta_v12_depth_and_selected(
    ...
    *,
    ...
    if int(s_sub) != 4:
        raise ValueError(f"v12 depth/selected requires S=4, got {s_sub}")
    k_clusters = int(selected_i8.shape[-1])
    n_rows = int(row_stride if rows is None else rows)
    _ta_v12_depth_selected_kernel[(int(h_q),)](
        order_i64,
        sorted_scores,
        threshold_f32,
        depth_i32,
...
def run_ta_v12_corrected_cluster_attn(
    q: torch.Tensor,
    order_i64: torch.Tensor,
    depth_i32: torch.Tensor,
    selected_i8: torch.Tensor,
    ...
    *,
    ...
    if int(s_sub) != 4 or int(bf) != 4:
        raise ValueError(f"v12 corrected requires S=4, bf=4; got S={s_sub}, bf={bf}")
    ...
    _ta_v12_corrected_cluster_attn_kernel[(int(q.shape[0]), int(num_splits))](
        q,
        order_i64,
        depth_i32,
        selected_i8,
