"""TA_attention_v10.0 — S=4, bf=4 cluster-streaming attention.

This path targets the low checked-fraction regime.  It keeps exact TA depth
selection, then streams the selected cluster children directly:

  centroid_scores -> sort -> stop_depth -> selected mask
                  -> cluster-stream attention -> reduce

The attention kernel does not build ``cand_b`` / ``cand_kvh`` and does not scan
``N_pad``.  Duplicate keys selected by multiple subspaces are assigned to the
first selected subspace, matching the unique-key candidate mask semantics.
"""
...
from ._TA_triton_v10_0 import (
    HAS_TRITON,
    run_ta_v10_cluster_attn,
...
def _workspace(
    ...
    *,
    ...
    cache = state.setdefault("_TA_attention_v10_0_cache", {})
...
def _capture_pipeline(ws: dict, launch_kwargs: dict, state: dict) -> None:
    if ws["graph"] is not None or ws["capture_failed"]:
        return
    if not bool(state.get("_TA_attention_v10_0_use_cuda_graphs", True)):
...
def attend(
    ...
    del keys_children
    if not HAS_TRITON:
        raise RuntimeError("TA_attention_v10.0 requires Triton")
