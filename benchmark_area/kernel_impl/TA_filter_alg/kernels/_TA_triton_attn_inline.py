"""Split-N flash-style Triton attention with INLINE candidate-mask evaluation.

Used by ``TA_attention_v_1_2``: instead of materializing a (H_q, N_pad)
candidate mask in torch and shipping it to the kernel, we ship the cheaper
``selected`` tensor (H_q, S, K, int8) and the ``assigns_padded`` tensor
(S, H_kv_eff, N_pad, int32) and OR-reduce inline:

    is_cand[g, n] = OR_{s} selected[hq_vec[g], s, assigns_padded[s, kvh, n]]

Why this might be faster than precomputing the mask:
  * Saves a write of an (H_q, N_pad) int8 tensor (~hundreds of KB) and the
    matching read inside the attention kernel.
  * Saves the per-subspace gather + OR work in torch (S launches, one per
    subspace).
  * Tradeoff: the kernel does S small indexed loads per (chunk, group) — fine
    when S is small (8 or 16) and ``selected`` is tiny (kB-scale, well in L2).

Otherwise identical to the precomputed-mask variant: split-N programs, online
softmax in log2-score space (exp2), threshold-T mask on raw scores, optional
buffer fold, partials reduced afterwards.
"""
...
if HAS_TRITON:
    ...
    @triton.jit
    def _ta_attn_inline_kernel(
        Q_ptr,
        KeysT_ptr,                # (H_kv_eff, D, N_pad) fp16
        Values_ptr,               # (H_kv_eff, N_pad, D_v) fp16
        Selected_ptr,             # (H_q, S, K) int8 -- flat
        Assigns_ptr,              # (S, H_kv_eff, N_pad) int32 -- flat
        ...
        for n_chunk in range(n_start, n_end, BLOCK_N):
            ...
            cand = tl.zeros([GROUPS_POW, BLOCK_N], dtype=tl.int32)
...
def run_ta_attn_inline(
    ...
    _ta_attn_inline_kernel[grid](
        ...
        n_pad,
        k_clusters,
        h_kv_eff * n_pad,        # ASSIGN_STRIDE_S
        s_sub * k_clusters,      # SELECTED_STRIDE_HQ
        k_clusters,              # SELECTED_STRIDE_S
        h_kv_eff,
