"""TA_attention_v13.4 - parent mask with arithmetic child expansion.

Requires ``TA_build_v13.4``.  That build makes parent ``p`` own key rows
``p * 4 : (p + 1) * 4`` for every subspace.  This kernel therefore keeps only
the selected-parent mask from sort+depth and fuses child-mask expansion into
the attention scan with ``parent = key // 4``.  No parent->children lookup is
loaded in the timed attention path.
"""
...
from ..commons._TA_triton_v12 import run_ta_v12_depth_and_selected
from ._TA_triton_v13_4 import HAS_TRITON, run_ta_v13_4_parent_block_attn
from ..v13.TA_attention_v_13_0 import (
    _ATTN_BLOCK_N,
    _ATTN_SPLITS,
    _ATTN_STAGES,
    _ATTN_WARPS,
...
from ..v13.TA_attention_v_13_2 import attend as attend_v13_2
...
def _workspace(
    ...
    *,
    ...
    cache = state.setdefault("_TA_attention_v13_4_cache", {})
    key = (device.index, h_q, s_sub, k_clusters, n_pad, d_v, d, _ATTN_SPLITS, _PARENT_BLOCK_P)
    if cache.get("key") != key:
        ...
        cache["parent_mask"] = torch.empty(h_q, s_sub, k_clusters, device=device, dtype=torch.int8)
...
def _launch_pipeline(
    *,
    ...
    run_ta_v13_4_parent_block_attn(
        q=static_q,
        parent_mask_i8=ws["parent_mask"],
...
def _capture_pipeline(ws: dict, launch_kwargs: dict, state: dict) -> None:
    if ws["graph"] is not None or ws["capture_failed"]:
        return
    if not bool(state.get("_TA_attention_v13_4_use_cuda_graphs", True)):
...
def attend(
    ...
    del keys_children
    if not HAS_TRITON:
        raise RuntimeError("TA_attention_v13.4 requires Triton")

    if not bool(state.get("arithmetic_parent_children", False)):
        if "parent_children_i32" in state:
            return attend_v13_2(q, threshold, state, buffer_keys, buffer_values, q_head_to_kv, scale)
