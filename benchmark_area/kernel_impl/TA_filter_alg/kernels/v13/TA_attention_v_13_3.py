"""TA_attention_v13.3 — capped top-L stamped mask.

Same as v13.2, but replaces full sort with sorted top-L parent retrieval.
For the 8k/12k capture path this scans 256 rows for the TA stop depth instead
of the full K array.
"""
...
from ..commons._TA_triton_v13 import (
    HAS_TRITON,
    run_ta_v13_depth_count_stamp_rows,
    run_ta_v13_mark_stamp_rs,
...
def _workspace(
    ...
    *,
    ...
    cache = state.setdefault("_TA_attention_v13_3_cache", {})
    key = (device.index, h_q, s_sub, k_clusters, l_top, n_pad, d_v, d, _ATTN_SPLITS, _ATTN_BLOCK_N)
    if cache.get("key") != key:
        ...
        cache["top_scores"] = torch.empty(h_q, s_sub, l_top, device=device, dtype=torch.float16)
        cache["top_order"] = torch.empty(h_q, s_sub, l_top, device=device, dtype=torch.long)
...
def _capture_pipeline(ws: dict, launch_kwargs: dict, state: dict) -> None:
    if ws["graph"] is not None or ws["capture_failed"]:
        return
    if not bool(state.get("_TA_attention_v13_3_use_cuda_graphs", True)):
...
def attend(
    ...
    del keys_children
    if not HAS_TRITON:
        raise RuntimeError("TA_attention_v13.3 requires Triton")
    ...
    l_top = _topl_for_n(n_real, k_clusters)
