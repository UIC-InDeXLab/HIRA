"""TA_attention_v13.1 — stamped mask + packed candidate attention.

Keeps v13's parent-contiguous child lookup, but fuses mask marking with
candidate compaction.  A stamped visited mask deduplicates keys via atomics;
new keys are packed into ``cand_ids``.  Attention then streams only packed
candidate ids instead of scanning all ``N_pad`` mask positions.
"""
...
from ..commons._TA_triton_v11 import run_ta_v11_4_flat_attn
from ..commons._TA_triton_v13 import (
    HAS_TRITON,
    run_ta_v13_depth_count_stamp,
    run_ta_v13_pack_candidates_stamp,
)
...
def _workspace(
    ...
    *,
    ...
    cache = state.setdefault("_TA_attention_v13_1_cache", {})
    key = (
        device.index,
        h_q,
        s_sub,
...
def _launch_pipeline(
    *,
    ...
    run_ta_v13_pack_candidates_stamp(
        ...
        num_splits=_PACK_SPLITS,
    )
    run_ta_v11_4_flat_attn(
        q=static_q,
        flat_ids_i32=ws["cand_ids"],
        flat_count_i32=ws["counts"],
...
def _capture_pipeline(ws: dict, launch_kwargs: dict, state: dict) -> None:
    if ws["graph"] is not None or ws["capture_failed"]:
        return
    if not bool(state.get("_TA_attention_v13_1_use_cuda_graphs", True)):
...
def attend(
    ...
    del keys_children
    if not HAS_TRITON:
        raise RuntimeError("TA_attention_v13.1 requires Triton")
