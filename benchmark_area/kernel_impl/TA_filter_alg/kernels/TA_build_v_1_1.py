"""TA_build_v1.1 — capacity-capped TA-filter index.

This version keeps the v1.0 state contract and adds fixed-size child lists for
gathered attention kernels.  Unlike v1.0's unconstrained k-center assignment,
each cluster owns at most ``bf`` real keys, matching the TA-filter algorithm
assumption and enabling a compact (S, H, K, bf) traversal layout.
"""
...
def _assigns_dtype(k: int) -> torch.dtype:
    return torch.int16 if k < 32768 else torch.int32


def _dominant_axis(points: torch.Tensor) -> torch.Tensor:
    """Fast PCA-tree split axis: coordinate with largest local variance."""
    _, d = points.shape
    if points.shape[0] <= 1:
...
def _balanced_pca_tree_subspace(keys_sub: torch.Tensor, bf: int) -> tuple[torch.Tensor, torch.Tensor]:
    """Capacity-constrained subspace clustering.

    The tree split gives exact target leaf sizes in ``[1, bf]``.  We avoid a
    full balanced Lloyd refinement here because the first fast-attention target
    is latency; build quality can be iterated independently once the gathered
    attention path is in place.
    """
...
def _children_from_assign(
    ...
    *,
    ...
    """Build fixed child lists from exact-capacity assignments."""
    h, n = assign.shape
    ...
    for head in range(h):
        ...
        if bool((counts > bf).any().item()):
            ...
            raise RuntimeError(f"TA_build_v1.1 capacity violation: max cluster size {max_count} > bf={bf}")
...
def build(
    ...
    for start, end in slices:
        ...
        ap[:, :n_real] = assign
        assigns_padded_list.append(ap.to(_assigns_dtype(k)).contiguous())
