"""TA_attention_v5.2 — v4.0 with lower-overhead candidate marking.

This keeps the winning v4.0 dense-mask attention design but retunes the mask
marker for the observed shallow depths.  Fewer marker programs reduce launch
work and atomics/duplicate stores; attention geometry remains the v4.0 winner.
"""
...
def _workspace(state: dict, *, h_q: int, s_sub: int, k_clusters: int, n_pad: int, d_v: int, device) -> dict:
    cache = state.setdefault("_TA_attention_v5_2_cache", {})
...
def attend(
    ...
    del keys_children
    if not HAS_TRITON:
        raise RuntimeError("TA_attention_v5.2 requires Triton")
