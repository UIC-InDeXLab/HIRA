"""TA_attention_v11.1 — S=4, bf=4 top-L per-subspace cluster-streaming attention.

Variant of v11.0 that replaces sort + stop_depth with a per-subspace top-L
retrieval over centroid scores.  No threshold filter on keys; every key
inside one of the top-L clusters per subspace contributes to softmax.

L is hardcoded by index size N (= cluster keys before padding):

    N <= 4096                : L = 256
    4096 < N <= 12288        : L = 512
    N > 12288  (incl. >= 20k): L = 1024

Always clamped to ``min(L, K)`` where K is the cluster count.
...
from ..commons._TA_triton_v11 import (
    HAS_TRITON,
    run_ta_v11_build_selected_from_topl,
...
def _topl_for_n(n_real: int, k_clusters: int) -> int:
    if n_real <= 4096:
        l = 256
    elif n_real <= 12288:
        l = 512
...
def _workspace(
    ...
    *,
    ...
    cache = state.setdefault("_TA_attention_v11_1_cache", {})
    key = (device.index, h_q, s_sub, k_clusters, l_topl, n_pad, d_v, d, _ATTN_SPLITS)
    if cache.get("key") != key:
        ...
        cache["topk_values"] = torch.empty(h_q, s_sub, l_topl, device=device, dtype=torch.float16)
        cache["topk_indices"] = torch.empty(h_q, s_sub, l_topl, device=device, dtype=torch.long)
        cache["depth"] = torch.full((h_q,), int(l_topl), device=device, dtype=torch.int32)
...
def _launch_pipeline(
    *,
    ...
    # Per-subspace top-L cluster ids; values written into pre-alloc buffer.
    torch.topk(
        ws["scores"],
        k=int(ws["topk_indices"].shape[-1]),
        dim=-1,
        sorted=False,
        out=(ws["topk_values"], ws["topk_indices"]),
    )
    run_ta_v11_build_selected_from_topl(
        ws["topk_indices"],
        ws["selected"],
...
def _capture_pipeline(ws: dict, launch_kwargs: dict, state: dict) -> None:
    if ws["graph"] is not None or ws["capture_failed"]:
        return
    if not bool(state.get("_TA_attention_v11_1_use_cuda_graphs", True)):
...
def attend(
    ...
    del keys_children
    if not HAS_TRITON:
        raise RuntimeError("TA_attention_v11.1 requires Triton")
    ...
    l_topl = _topl_for_n(n_real, k_clusters)
