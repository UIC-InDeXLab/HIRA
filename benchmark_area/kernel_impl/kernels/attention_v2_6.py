"""attention_v2.6 — Fuses buffer scan into the index kernel.

Instead of index_kernel → reduce+buffer_kernel (2 launches), this uses
index+buffer_kernel → reduce_kernel.  The buffer columns are distributed
across splits so each thread block processes its share.  The reduce kernel
is the simpler split-only version (no buffer handling).

Combines with exp2 optimization.  Uses num_splits=85.
"""
...
def _capture_graph(state, stage, launch_fn, launch_args):
    if stage["graph"] is not None or stage["capture_failed"]:
        return
    if not bool(state.get("_attn_v2_6_use_cuda_graphs", True)):
...
def attend(
    ...
    del keys_children
    if not HAS_TRITON:
        raise RuntimeError("attention_v2_6 requires Triton")
    if "keys_reord" not in state:
        raise RuntimeError("attention_v2_6 requires build_v2-style state")
    ...
    cache = state.setdefault("_attn_v2_6_fixed", {})
